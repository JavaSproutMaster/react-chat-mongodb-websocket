...
export default function Pages() {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BASE_URL}/admin/pages?page=${page}&size=${limit}`, {
            headers: {
    ...
    const paginationData = async (currentPage) => {
        const res = await fetch(
            `${process.env.REACT_APP_BASE_URL}/admin/pages?size=${limit}&page=` + currentPage, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ` + localStorage.getItem('accessToken')
            },
    ...
    const HandleDelete = (id) => {
        ...
        if (window.confirm("Are you sure you want to delete?")){
            axios.delete(`${process.env.REACT_APP_BASE_URL}/admin/pages/${id}`, {
                headers: {
                'Accept': 'application/json, text/plain, */*',
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ` + localStorage.getItem('accessToken')
            ...
            <div className="flex flex-wrap min600">
                <div className="w-full mb-12 xl:mb-0 px-4 padding-top80">
                    <div className="grid grid-cols-2 gap-4 mb-2">
                        ...
                        <div className='mt-4'>
                            <Link to='/admin/add-page' type="submit" className="text-white float-right bg-success font-medium rounded px-5 py-2.5 text-center">Add Pages</Link>
                        </div>
                    </div>
                            ...
                            <tbody>
                                {details && details.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td width="30%">{(item.type.replace("_", " ")).replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase())}</td>
                                        <td width="20%" className='text-center'>
                                            <Link to={`/admin/view-page/${item._id}`} className="focus:outline-none text-white bg-package-maroon font-medium rounded-lg text-sm p-2.5 mr-2">
