...
export default function TransactionHistory() {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BASE_URL}/admin/payments?page=${page}&size=${limit}`, {
            headers: {
    ...
    const paginationData = async (currentPage) => {
        const res = await fetch(
            `${process.env.REACT_APP_BASE_URL}/admin/payments?size=${limit}&page=${currentPage}`, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ` + localStorage.getItem('accessToken')
            },
    ...
    const getFilterData = ({ client, advisor, created_date, created_date_to }) => {
        ...
        axios.get(`${process.env.REACT_APP_BASE_URL}/admin/payments?page=1&filter_client=${client}&filter_advisor=${advisor}&filter_created_from=${created_date}&filter_created_to=${created_date_to}`, {
            headers: {
                'Accept': 'application/json, text/plain, */*',
                'Content-Type': 'application/json',
                'Authorization': `Bearer ` + localStorage.getItem('accessToken')
