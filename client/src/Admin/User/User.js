...
export default function User() {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BASE_URL}/admin/users?page=${page}&size=${limit}`, {
            headers: {
    ...
    const paginationData = async (currentPage) => {
        const res = await fetch(
            `${process.env.REACT_APP_BASE_URL}/admin/users?size=${limit}&page=` + currentPage, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ` + localStorage.getItem('accessToken')
            },
    ...
    const HandleDelete = (id) => {
        ...
        if (window.confirm("Are you sure you want to delete?")){
            axios.delete(`${process.env.REACT_APP_BASE_URL}/admin/users/${id}`, {
                headers: {
                'Accept': 'application/json, text/plain, */*',
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ` + localStorage.getItem('accessToken')
    ...
    const getFilterData = ({ status, name, email, username, created_date }) => {

        axios.get(`${process.env.REACT_APP_BASE_URL}/admin/users?filter_status=${status}&page=1&filter_name=${name}&filter_email=${email}&filter_username=${username}&filter_created_at=${created_date}`, {
            headers: {
                'Accept': 'application/json, text/plain, */*',
                'Content-Type': 'application/json',
                'Authorization': `Bearer ` + localStorage.getItem('accessToken')
            ...
            <div className="flex flex-wrap min600">
                <div className="w-full mb-12 xl:mb-0 px-4 padding-top80 gridstyle">
                    <div className="grid grid-cols-2 gap-4 mb-2">
                        <div className='mt-4'>
                            <h1 className='text-2xl text-black font-bold mb-3'>Users</h1>
                        </div>
                        <div className='mt-4'>
                            <Link to='/admin/add-user' type="submit" className="text-white float-right bg-success font-medium rounded px-5 py-2.5 text-center">Add User</Link>
                        </div>
                    </div>
                            ...
                            <tbody>
                                {details && details.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{item.username}</td>
                                        <td>{item.email}</td>
                                        <td>{(item.group && item.group.name) ? item.group.name : 'Admin'}</td>
                                        <td>{item.status === 0 ? 'Not Active' : 'Active'}</td>
                                        <td className='text-center flex'>
                                            <Link to={`/admin/view-user/${item._id}`} className="focus:outline-none text-white bg-package-maroon font-medium rounded-lg text-sm p-2.5 mr-2">
